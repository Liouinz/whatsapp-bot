...
- **Web-Oberfläche**: QR-Login, Gruppenliste mit Profilbildern, Pro-Gruppen-
  Einstellungen, Live-Dashboard – alles passwortgeschützt und responsiv.
...
| `/status`       | offen        | JSON-Status                                     |
| `/qr?key=…`     | `QR_PASSWORD`| WhatsApp-Login-QR-Code                          |
| `/settings?key=…` | `QR_PASSWORD`| Gruppenübersicht                              |
| `/group?id=…&key=…` | `QR_PASSWORD`| Pro-Gruppen-Konfiguration                   |
| `/dashboard?key=…` | `QR_PASSWORD`| Live-Server-Daten                            |
